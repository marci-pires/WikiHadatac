...
- In Blazegraph: the content of the store_users
- In Solr (be sure to shutdown Solr before erasing its contents): and content of the following tables under solr/solr-home:
  - users
  - linked_account
  - token_action 

One easy way of erasing the content from store_users' namespace in Blazegraph is of erasing the namespace itself and creating in again.

Each table in Solr is composed of two folders and one file: __conf__, __data__ and __core.properties__. The content of the table is inside the __data__ folder. In this case, it is possible to erase the __data__ folder that is going to be recreated the next time you start Solr. Another option is to replace the entire solr/solr_home folder with its content from github.
