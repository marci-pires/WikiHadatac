...
In production, a software upgrade DOES NOT replace the content of __hadatac/conf__ in the distribution because the upgrade should preserve current configuration. To change any configuration file, one needs to change the configuration file in both the current distribution and in "/data/conf" that should not be in "[HADatAc]/conf".  
...
This configuration file specifies the folders where uploaded files are stored when they are initially uploaded into HADatAc, and where they are stored after HADatAc has processed them. Data Files and metadata files that are uploaded into HADatAc are managed as part of the overall content of the app. 

__IMPORTANT: the folder paths identified in this configuration file MUST BE OUTSIDE of the HADatAc distribution folder so that the files are not affected during HADatAc's software upgrade.__
