...
Authentication is done through email verification, which requires HADatAc to communicate with users through emails. The configuration file is __smtp.conf__ under __/conf/play_authenticate__. Instructions for filling up the configuration file are described inside of the file itself.
