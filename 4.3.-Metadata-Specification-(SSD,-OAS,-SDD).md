...
Reference is the name used mainly in SDDs to refer to semantic object collections defined in an SSD. For example, let say that children are defined in an SSD and mothers of these children are defined just in SDDs, but that it is important for the mothers to be related to the children. In this sense, it is possible in SDDs to refer to SSD objects using the reference field.

Within the stope of studies, certain categories of objects are well defined by a given term, while others depend of their relationship to other objects. For instance, in some studies, the terms "Child" and "Mother" may be complete and precise enough to full identify the nature of some objects of concern. In these same studies, the term "Sample" is not complete enough to identify the nature of some sample objects, since they may be samples from a child, or samples from a mother, for example. In the case described above, the SOCs of Children and Mothers are grounded by the terms "Child" and "Mother" while the SOC Sample may not have any grounding label. In this case, it is important for non-grounded objects to be logically connected to one grounded object.
...
study | data-file-name | data-dictionary | deployment-uri | row-scope | cell-scope | owner-scope | permission-url
----- | -------------- | --------------- | -------------- | --------- | ---------- | ----------- | --------------
2016-1234 | demographics | SDD-demographics | proj:quest  | SOC-LOCATIONS |      | joe@example.org | http://example\#team
