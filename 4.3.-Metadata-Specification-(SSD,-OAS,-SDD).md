...
Objects need to be located in a pre-existing RDF graph of interest to have the value of their properties updated. 
The object could be anything that, for instance, can be instantiated on demand into any RDF repository, given that we were sure that we did not know anything else about such object. However, it can also be the case that the object is one that already exists in the RDF repository, and that such repository may already have some other attributes assigned for that given object. 
