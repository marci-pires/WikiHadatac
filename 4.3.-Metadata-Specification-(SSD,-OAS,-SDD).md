...
* a name-template for selecting incoming files (``data-file-name" attribute in Table \ref{tab:OASSpecification}); 
* the data dictionary of choice for processing the file(s) (``data-dictionary" attribute in Table \ref{tab:OASSpecification}) used to process the data ingestion; 
* the scope composed of the ``row scope" attribute and ``cell scope" attribute;  
* the email address of the person who is the actual owner of the data; and
* permission policy for accessing the data.

The data dictionary of the OAS identifies the SDD document that describes how objects identified during the parsing of the SDD file content map to object types. 
The precise identification of which object is mapped to each value is specified by the row-scope attribute and cell-scope attribute described below. 

Column | Description
-------| -----------
data-file-name | a mask name for data files 
data-dictionary | SDD for data interpretation
row-scope | row level scope
cell-scope | cell level scope
owner-email | data owner 
permission-url | data access policy

An Example

data-file-name | data-dictionary | row-scope | cell-scope | owner-scope | permission-url
-------------- | --------------- | --------- | ---------- | ----------- | --------------
demographics2* | SDD-demographics |          |            | joe@example.org | http://example\#team

Table \ref{tab:OASSpecification} shows an example of how our SDD-demographics described in Section \ref{sec:SOCGen} is assigned to parse the content of the file \texttt{demographics2.csv} in Figure \ref{fig:dataInCSV}. 
The OAS specification identifies that every file named ``demographics2*" that is submitted for processing, including \texttt{demographics2\_2.csv} (cf. Table \ref{tab:demographics22}), will be processed with the help of SDD-demographics (composed of DASA and DASO).
The OAS further specifies that joe@example.org is the email of the owner of the data content of any file that starts with the name ``demographics2", and that anyone with permission \url{http://example#team} has access to content extracted from any file using this OAS.
