...
sheet |	hasURI	| type	| isMemberOf | hasScope | hasTimeScope | hasSpaceScope | cardinality	| role | Reference | Grounding Label
----- | ------- | ----- | ---------- | -------- | ------------ | ------------- | -----------	| ---- | --------- | ---------------  
| | kb:STD-A | hasco:Study | | | | | | | |  
#SOC-SUBJECTS | kb:SOC-SUBJECTS	| hasco:SubjectGroup | kb:STD-A | | | | 6 | ont:isSubjectOf  | ??child | Child  
#SOC-SAMPLES | kb:SOC-SAMPLES | hasco:SampleCollection | kb:STD-A | kb:SOC-SUBJECTS | | | 1 | ont:isSampleOf | ??sample |    

Grounding Label identifies that the nature of objects of a given collection is fully identified by the value of this field. The nature of objects of of collections without grounding labels require the use of grounding labels of associated object to understand the value of this field. Using the SDD above, if a subject has an attribute "Height", the attribute is identified by "Child Human Height". If a blood sample has an attribute "Concentration Lead", the attribute is identified by "Child Blood Sample Concentration Lead". In this case, "Blood Sample" is derived from the kind of entity of the object within #SOC-SAMPLES, and "Child" is derived from the hasScope relationship between #SOC-SAMPLES and #SOC-SUBJECTS".      
...
OAS specifications identify resources necessary for processing content of CSV files. An OAS is a CSV file itself composed of one row of data (i.e., a record) composed of the following six fields (columns):

Column Header | Column Description
--------------| ------------------
data-file-name | a name-template for selecting incoming files 
data-dictionary | the data dictionary of choice for processing the file(s)
row-scope | row level scope (see explanation below in this section)
...
It is a value assignment strategy used when all the columns of a CSV file are direct and indirect references to attributes of a single object type, when objects are grouped in SOCs, and when one identifier is available to select one object in the SOC.
...
data-file-name | data-dictionary | row-scope | cell-scope | owner-scope | permission-url
-------------- | --------------- | --------- | ---------- | ----------- | --------------
demographics   | SDD-demographics | SOC-LOCATIONS |            | joe@example.org | http://example\#team
...
## 4.3.3.1. SDD Verification Rules
...
__Error message__: “readCatalog exception”  
__Correction__: Include the missing info sheet  

### SDD-Level Rule 2 
SDD must contain the following sheets: dictionaryFile, codeBookFile, timelineFile, and codeMappingFile.  
__Consequence of breaking the rule__:  HADatAc will skip the File and generate an error message but will not stop processing the SDD  
...
__Correction__: Add the missing sheet(s)  
...
### SDD-Level Rule 4  
DataDictionary sheet cannot be empty
__Consequence of breaking the rule__:  HADatAc will stop processing the SDD  
__Error messages__: “DataDictionary sheet is empty”  
__Correction__: Add content into data dictionary sheet  
