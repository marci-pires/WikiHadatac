...
OAS specifications identify resources necessary for processing the content from CSV files. Basically, an OAS is composed of the following fields:
...
\begin{table}[H]
\centering
\caption{An Object Access Specification (or  OAS file).}
\begin{tabular}{l|l|l}
Attribute & Value & Description \\
\hline
data-file-name & demographics2* & a mask name for data files \\ \hline
data-dictionary & SDD-demographics & SDD for data interpretation \\ \hline
row-scope &  &  row level scope \\  \hline
cell-scope &  &  cell level scope \\  \hline
owner-email & joe@example.org & data owner \\ \hline
permission-url & http://example\#team & data access policy \\ \hline
\end{tabular}
\label{tab:OASSpecification}
\end{table}
