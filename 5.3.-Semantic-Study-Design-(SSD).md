...
Study objects without explicitly assigned identifiers are often specified on [SDDs]() and created on demand during data ingestion. Thus, we are talking about objects that are not defined in an SSD. Nevertheless, these objects are still part of the same study, and they are very often related to objects defined inside SOCs. Virtual columns are used to refer to study objects created during data ingestion to SDD-defined objects.
