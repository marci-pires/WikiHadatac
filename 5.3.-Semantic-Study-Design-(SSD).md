...
Virtual columns are used to refer to study objects created during data ingestion and specified inside SDDs. For the __Study A__ mentioned in [Step 1](https://github.com/paulopinheiro1234/hadatac/wiki/5.3.-Semantic-Study-Design-(SSD)#step-1-identify-semantic-object-collections-soc), let say that some files may have data about the household of subjects in __kb:SOC-SUBJECTS__. In this case, the table below illustrates an updated version on the table in [Step 1](https://github.com/paulopinheiro1234/hadatac/wiki/5.3.-Semantic-Study-Design-(SSD)#step-1-identify-semantic-object-collections-soc) showing the specification of the __kb:SOC-HOUSEHOLDS__ virtual column for households.  
...
This virtual column is saying that the reference for households within __Study A__ and specified inside of SDDs should be __??household__. 

A virtual column is said to be well-formed if the following is observed:

* the virtual column has values for __hasUri__, __hasSOCReference__ and __groundingLabel; and
* the virtual column has no values for __sheet__ and __type__. 
