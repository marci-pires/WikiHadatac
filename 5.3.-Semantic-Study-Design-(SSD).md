...
Study objects without explicitly assigned identifiers are often specified on [SDDs](https://github.com/paulopinheiro1234/hadatac/wiki/5.4.-Semantic-Data-Dictionary-(SDD)) and created on demand during data ingestion. Thus, we are talking about objects that are not defined in an SSD. Nevertheless, these objects are still part of a study, and as such, they may be very often related to objects defined in SDDs (as described below in Section ). 

Virtual columns are used to refer to study objects created during data ingestion to SDD-defined objects.
