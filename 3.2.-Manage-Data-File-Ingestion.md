...
The final go of ingesting the content of a study is go get the data of the study ingested. The data can be either fed by a collection of data files (DA files), or fed through a message stream, or a combination of both). In the diagram below we see the blue box as the collection of metadata files required to ingest data fed through message streams. The grey box (which includes everything inside of the blue box) is the collection of metadata files required to ingest data files.
 
![](https://raw.githubusercontent.com/paulopinheiro1234/hadatac-screenshots/master/Sec3/StudyMetadataFiles.png)

The diagram above shows how one category of metadata file depends on another category of metadata files. For example, and SSD file can only be ingested after its corresponding STD file has been ingested, and a DA file can only be ingested when its corresponding STRs files have been ingested. In that way, we can see, for example, that STD, SDD and DPL files can be ingested at any time, since they do not depend of other metadata files.
